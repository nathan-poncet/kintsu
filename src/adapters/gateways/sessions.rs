...
mod tests {
    ...
    fn a_gone_subscriber_is_dropped_and_messages_wait_again() {
        ...
        let waiting: Vec<String> = s
            ...
            .collect();
        // The first write to a peer that just closed may still succeed and
        // vanish; the failure is seen on the next one, which then waits.
        assert_eq!(waiting.last().map(String::as_str), Some("second"));
    }
